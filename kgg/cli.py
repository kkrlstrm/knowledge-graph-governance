...
def cmd_ingest(argv):
    ...
    if not args.apply:
        ...
        if held:
            keys = ",".join(p.key for p in held)
            print(f"  {len(held)} item(s) need approval:")
            print(f"    kgg approve {result['run_id']} --items {keys} --approver <you> --proposals {args.proposals} --audit {args.audit}")
...
def main():
    ...
    if not argv or argv[0] in ("-h", "--help"):
        print("kgg <command>\n\n  ingest <file> --schema S [--backend DSN] [--apply]\n"
              "  approve <run_id|--file F> --approver NAME [--items a,b | --all]\n"
